<template>
    <div class="flex flex-col min-h-screen w-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 overflow-x-hidden">
      ...
      <main class="flex-1 flex flex-col items-center pt-24 px-6 w-full">
        <div class="w-full max-w-2xl bg-white/20 backdrop-blur-md rounded-2xl shadow-md p-6 space-y-4">
          <h2 class="text-2xl font-bold text-gray-800">Chat with @{{ username }}</h2>
          ...
          <div class="flex flex-col space-y-2 max-h-[400px] overflow-y-auto">
            <div
                ...
                :key="msg.id"
                :class="msg.sender === currentUser.username ? 'self-end text-right' : 'self-start text-left'"
                >
                <div class="inline-block px-4 py-2 rounded-xl text-white"
                    :class="msg.sender === currentUser.username ? 'bg-purple-600' : 'bg-gray-500'">
                    {{ msg.message }}
                </div>
  ...
  const fetchMessages = async () => {
    try {
      const res = await authAxios.get(`/messages/thread/?with=${username}`)
      messages.value = res.data.reverse() // show oldest first
    } catch (error) {
  ...
  const sendMessage = async () => {
    ...
    try {
      await authAxios.post('/messages/send/', {
        receiver: username,
