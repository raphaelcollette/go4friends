<template>
    <div class="flex flex-col min-h-screen w-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 overflow-x-hidden">
      ...
      <main class="flex-1 flex flex-col items-center pt-24 px-6">
        ...
        <!-- Events List -->
        <div v-if="events.length > 0" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 w-full max-w-6xl">
          <div
          ...
          >
            ...
            <div class="absolute top-2 left-2">
              <span
                v-if="event.club"
                class="bg-purple-500 text-white text-xs font-bold py-1 px-2 rounded-lg"
              >
                {{ event.club }}
              </span>
              <span
                v-else
                class="bg-green-500 text-white text-xs font-bold py-1 px-2 rounded-lg"
              >
                School Event
              </span>
            </div>
